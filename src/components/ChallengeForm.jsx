...
const ChallengeForm = () => {
  ...
  const [states, setStates] = useState([]); // Estado para armazenar os estados do Brasil obtidos da API
  const [cities, setCities] = useState([]); // Estado para armazenar as cidades do Brasil obtidas da API
  ...
  const [selectedDate, setSelectedDate] = useState(null); // Estado para armazenar a data selecionada no datepicker
  const [showPassword, setShowPassword] = useState(false); // Estado para controlar a visibilidade da senha
  const [showConfirmationPassword, setShowConfirmationPassword] =
    useState(false); // Estado para controlar a visibilidade da confirmação de senha
  const [termsAccepted, setTermsAccepted] = useState(false); // Estado para armazenar se os termos e condições foram aceitos
  const [errorMessage, setErrorMessage] = useState(""); // Estado para armazenar a mensagem de erro
  const [showFormSent, setShowFormSent] = useState(false); // Estado para controlar a exibição do formulário enviado com sucesso
  const [modalOpen, setModalOpen] = useState(false); // Estado para controlar a abertura/fechamento do modal de termos e condições
  const [formSentSuccess, setFormSentSuccess] = useState(false); // Estado para controlar se o formulário foi enviado com sucesso
  const [isErrorFixed, setIsErrorFixed] = useState(false); // Estado para controlar se o erro está fixo na tela
  const errorRef = useRef(null); // Referência para o elemento de erro
  const campoRef = useRef(null); // Referência para o primeiro campo do formulário

  // Função chamada ao clicar no ícone de mostrar/esconder senha
  const handleShowPassword = () => {
    setShowPassword(!showPassword);
  };

  // Função chamada ao clicar no ícone de mostrar/esconder confirmação de senha
  const handleShowConfirmationPassword = () => {
    setShowConfirmationPassword(!showConfirmationPassword);
  };

  // Função chamada ao abrir o modal de termos e condições
  const openModal = () => {
    setModalOpen(true);
  };

  // Função chamada ao fechar o modal de termos e condições
  const closeModal = () => {
    setModalOpen(false);
  };

  // Função chamada ao marcar/desmarcar o checkbox de termos e condições
  const handleTermsChange = (event) => {
    setTermsAccepted(event.target.checked);
  };

  // Função chamada ao clicar no botão de resetar
  const handleReset = () => {
    localStorage.removeItem("formsData"); // Remove os dados armazenados no localStorage
    const storedData = JSON.parse(localStorage.getItem("formsData")) || [];
    console.log("BD Limpo: ", storedData);
  };

  // Função para obter a lista de estados do Brasil da API
  useEffect(() => {
  ...
  // Função para obter a lista de cidades do estado selecionado da API
  useEffect(() => {
  ...
  // Função para fechar o modal ao clicar fora ou pressionar a tecla ESC
  useEffect(() => {
  ...
  // Função para deixar primeiro campo do formulário focado ao abrir tela
  useEffect(() => {
  ...
  const handleSubmit = (event) => {
    ...
    // Deixa mensagem de erro flutuante relativa ao ponto da tela que eu estiver
    setIsErrorFixed(true);
    if (errorRef.current) {
      errorRef.current.scrollIntoView({ behavior: "smooth", block: "start" });
    }

    // Faz com que a mensagem de erro desapareça após 5 segundos
    setTimeout(() => {
      setErrorMessage("");
